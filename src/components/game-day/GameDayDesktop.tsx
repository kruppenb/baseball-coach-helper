...
import { LockToggle } from '../batting-order/LockToggle';
...
export function GameDayDesktop({ onPrintRequest, gameLabel, onDisplayStateChange }: GameDayDesktopProps) {
  ...
  const {
    ...
    isLocked: isBattingLocked,
    setLocked: setBattingLocked,
  } = useBattingOrder();
  ...
  }, [generatedLineups.length, generate, generateBattingOrder, presentPlayers.length, takeSnapshot, isBattingLocked]);
      ...
      {editor.lineup ? (
        <div className={styles.workspaceZone}>
          <Card label="Lineup">
            ...
            <ValidationPanel errors={editor.validationErrors} preErrors={preValidationWarnings} />
          ...
          <Card
            ...
            accessory={
              <LockToggle
                locked={isBattingLocked}
                onToggle={() => setBattingLocked(!isBattingLocked)}
              />
            }
          >
            {editor.battingOrder ? (
