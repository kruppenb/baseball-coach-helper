...
A web app for little league coaches to build fair, constraint-based inning-by-inning fielding lineups and continuous batting orders. The coach enters their roster (first name, last initial), pre-assigns pitchers and catchers, auto-generates the rest, and prints a single-page dugout card. Names are stored in browser localStorage only — never sent to a server.
...
- [ ] Roster input (first name, last initial) with localStorage persistence
...
- [ ] Continuous batting order (all kids bat regardless of fielding)
- [ ] Printable single-page dugout card (grid: columns = innings, rows = positions)
- [ ] Batting order displayed on dugout card
- [ ] Privacy: names stored in localStorage only, no server/cloud storage
...
- **Privacy**: Player names never leave the browser. No analytics, no server calls with PII.
...
|----------|-----------|---------|
| localStorage for roster | Coach doesn't want to re-enter names each session, but no server storage for privacy | — Pending |
